...
export const fetchHotels = createAsyncThunk(
  "hotelBanquet/fetchHotels",
  async (_, { rejectWithValue }) => {
    try {
      const res = await axios.get(
        `https://hotel-banquet.nearprop.in/api/hotels`
      );
...
// Get banquet by ID
export const fetchBanquetById = createAsyncThunk(
  "hotelBanquet/fetchBanquetById",
  async (banquetHallId, { rejectWithValue }) => {
    try {
      const res = await axios.get(
        `${baseUrl}/api/banquet-halls/${banquetHallId}`
      );
...
export const fetchRoomsByHotel = createAsyncThunk(
  "hotelBanquet/fetchRoomsByHotel",
  async (hotelId, { rejectWithValue }) => {
    try {
...
// Get reels by hotel
export const fetchHotelReels = createAsyncThunk(
  "hotelBanquet/fetchHotelReels",
  async (hotelId, { rejectWithValue }) => {
    try {
...
export const fetchBanquetReels = createAsyncThunk(
  "hotelBanquet/fetchBanquetReels",
  async (banquetHallId, { rejectWithValue }) => {
    try {
      const res = await axios.get(
        `${baseUrl}/api/reels/banquet/${banquetHallId}`
      );
...
export const fetchAverageRating = createAsyncThunk(
  "hotelBanquet/fetchAverageRating",
  async ({ type, id }, { rejectWithValue }) => {
    try {
      ...
      if (type === "banquet")
        url = `${baseUrl}/api/review/average?banquetId=${id}`;
...
const hotelBanquetSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchBanquets.fulfilled, (state, action) => {
        state.banquets = action.payload;
      })
      .addCase(fetchBanquetById.fulfilled, (state, action) => {
        state.banquetDetails = action.payload;
      })
      ...
      .addCase(fetchHotelReels.fulfilled, (state, action) => {
        state.reels = action.payload;
      })
      .addCase(fetchBanquetReels.fulfilled, (state, action) => {
        state.reels = action.payload;
      })
