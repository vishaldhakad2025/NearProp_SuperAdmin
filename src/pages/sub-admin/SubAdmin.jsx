...
import {
    Button, Table, Modal, Form, Input, Tag, message, Spin, Card,
    Space, Typography, Popconfirm, Divider, Row, Col, Image, DatePicker, Upload
} from 'antd';
import {
    FileAddOutlined, UserOutlined, EyeOutlined, DeleteOutlined, UploadOutlined
...
const AdvertisementManagement = () => {
    ...
    const { advertisements = [], advLoading = false, advCreating = false } =
    ...
    const handleCreate = async (values) => {
        ...
        const allFields = [
            "title", "description", "bannerImageUrl", "websiteUrl",
            "whatsappNumber", "phoneNumber", "emailAddress",
            "additionalInfo", "targetLocation", "latitude",
            "longitude", "radiusKm", "districtId"
        ];
    ...
    const columns = [
        {
            ...
            render: (url) =>
                url ? (
                    <Image src={url} width={100} height={60} style={{ objectFit: 'cover' }} preview={false} />
                ) : (
                    <Tag>No Image</Tag>
                ),
        },
        { title: 'Title', dataIndex: 'title', render: (t) => <Text strong>{t || '—'}</Text> },
        ...
        },
        {
            ...
            render: (_, rec) => (
                <Space>
                    <Button size="small" icon={<EyeOutlined />} onClick={() => openView(rec.id)} />
                    <Popconfirm
                        title="Delete?"
        ...
        <div style={{ padding: 24, background: '#f5f7fb', minHeight: '100vh' }}>
                ...
                title={
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <Space>
                            ...
                            <div>
                                <Title level={3} style={{ margin: 0 }}>My Advertisements</Title>
                                <Text type="secondary">Create and manage your ads</Text>
                            </div>
                        </Space>
                        <Space>
                            <Button onClick={() => dispatch(fetchAllAdvertisements())}>Refresh</Button>
            ...
            >
                {advLoading ? (
                    ...
                    <Table columns={columns} dataSource={myAds} rowKey="id" pagination={{ pageSize: 10 }} />
                )}
            </Card>
            ...
            >
                <Form form={form} layout="vertical" onFinish={handleCreate}>
                    ...
                    <div style={{ textAlign: 'right' }}>
                        ...
                        <Button type="primary" htmlType="submit" loading={advCreating} style={{ marginLeft: 8 }}>
                            Create
                        </Button>
                    </div>
                </Form>
                ...
                {viewOpen.ad && (
                    <>
                        {viewOpen.ad.bannerImageUrl && (
                            <Image src={viewOpen.ad.bannerImageUrl} style={{ width: '100%', borderRadius: 8 }} />
                        )}
                        <Divider />
                        <Row gutter={16}>
                            <Col span={12}><Text strong>Title:</Text> {viewOpen.ad.title}</Col>
                            <Col span={12}><Text strong>Location:</Text> {viewOpen.ad.targetLocation}</Col>
                            <Col span={12}><Text strong>WhatsApp:</Text> {viewOpen.ad.whatsappNumber}</Col>
                            <Col span={12}><Text strong>Phone:</Text> {viewOpen.ad.phoneNumber}</Col>
                            <Col span={24}><Text strong>Description:</Text><br />{viewOpen.ad.description}</Col>
                            <Col span={12}>
                                <Text strong>Valid From:</Text><br />
                                {moment(viewOpen.ad.validFrom).format('DD MMM YYYY, hh:mm A')}
                            </Col>
                            <Col span={12}>
                                <Text strong>Valid Until:</Text><br />
